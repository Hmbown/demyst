# Scilint Demonstration

This document showcases Scilint's ability to detect scientific integrity issues in Python code.
...
Running `scilint mirage examples/swarm_collapse.py` instantly flags the issue:
...
Running `scilint analyze examples/random_walk.py` produces a detailed JSON report:
...
Scilint can automatically generate a LaTeX methodology section guaranteeing that the code matches the paper.

**Command**:
```bash
scilint paper examples/swarm_collapse.py -o methodology.tex
```
...
**Scenario**: A kinematics calculation that accidentally adds time to distance—a physical impossibility.
