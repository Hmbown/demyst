...
class TestRandomWalkMirage:
    ...
    def test_detects_premature_discretization(self, random_walk_source, mirage_detector):
        ...
        disc_mirages = [m for m in mirage_detector.mirages if m['type'] == 'premature_discretization']
...
class TestAllExamplesMirageCount:
    ...
    @pytest.mark.parametrize("example_name,expected_min_mirages", [
        ("swarm_collapse", 1),
        ("random_walk", 2),
    ])
    def test_minimum_mirages_detected(self, examples_dir, example_name, expected_min_mirages):
        ...
        assert len(detector.mirages) >= expected_min_mirages, \
...
class TestMirageCLI:
    ...
    @pytest.mark.parametrize("example_file,should_fail", [
