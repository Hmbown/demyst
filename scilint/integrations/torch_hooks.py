        ...
        def hook(module, grad_input, grad_output):
            try:
                ...
                if stats.is_vanishing:
                    warnings.warn(
                        f"SCILINT: Vanishing gradient detected in {layer_name}. "
                        f"Mean gradient: {stats.mean:.2e}"
                    )
                if stats.is_exploding:
                    warnings.warn(
                        f"SCILINT: Exploding gradient detected in {layer_name}. "
        ...
        def hook(module, input, output):
            try:
                ...
                if saturation_ratio > 0.5:
                    warnings.warn(
                        f"SCILINT: High saturation in {layer_name}: {saturation_ratio:.1%}. "
                ...
                if dead_ratio > 0.5:
                    warnings.warn(
                        f"SCILINT: {dead_ratio:.1%} dead neurons in {layer_name}. "
