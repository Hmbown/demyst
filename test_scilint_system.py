#!/usr/bin/env python3
"""
Comprehensive system test for Scilint
...
from scilint.engine.transpiler import Transpiler, VariationTensor
...
'''
    ...
    assert len(transpiler.transformations) == 4, f"Expected 4 transformations, got {len(transpiler.transformations)}"
...
'''
    ...
    assert report.physics_tests_passed == report.physics_tests_total, "All physics tests should pass"
...
'''
    ...
    try:
        ...
        assert len(transpiler.transformations) >= 6, f"Expected at least 6 transformations, got {len(transpiler.transformations)}"
        ...
        assert 'VariationTensor' in transformed_code, "Transformed code should contain VariationTensor"
...
def test_self_application():
    ...
    # This is the ultimate test - Scilint refactoring itself
    transpiler = Transpiler()
    
    # Read the transpiler code itself
    transpiler_path = os.path.join(os.path.dirname(__file__), 'scilint', 'engine', 'transpiler.py')
    ...
    print(f"✅ Self-application test passed - found {len(transpiler.transformations)} self-transformations")
    return True
...
'''
    ...
    try:
        # Test CLI
        cmd = [
            sys.executable, '-m', 'scilint',
            '--target', temp_file,
            '--validation-suite', 'pytest'  # Use pytest suite which should pass
        ]
        
        result = subprocess.run(cmd, capture_output=True, text=True, cwd=os.path.dirname(__file__))
        
        # CLI should run successfully (may pass or fail validation depending on suite)
        assert result.returncode in [0, 1], f"CLI should complete with return code 0 or 1, got {result.returncode}"
...
def main():
    ...
    print("🚀 Running Scilint System Tests")
    print("=" * 50)
    ...
    if failed == 0:
        print("🎉 All system tests passed! Scilint is working correctly.")
        print("\n🌟 Key Achievements:")
        ...
        print("\n🎯 Scilint successfully prevents computational mirages!")
        return True
    else:
