...
class ScientificIntegrityReport:
    ...
    def to_markdown(self) -> str:
        ...
        lines.extend([
            f"![Status](https://img.shields.io/badge/scilint-{self.badge_status}-{badge_color})",
            "",
            "---",
            "",
            "## Detailed Results",
        ...
        for check in self.checks:
            icon = ":white_check_mark:" if check.passed else (
                ":x:" if check.severity == 'critical' else ":warning:"
            )
        ...
        lines.extend([
            "---",
            "",
            "*Report generated by [Scilint](https://github.com/scilint/scilint) - "
...
class CIEnforcer:
    ...
    def analyze_directory(self, directory: str,
                         ...
                         include_patterns: Optional[List[str]] = None) -> ScientificIntegrityReport:
        """
        ...
        def make_check(name: str, issues: List, critical_types: List[str]) -> IntegrityCheck:
            has_critical = any(
                i.get('severity') == 'critical' or i.get('type') in critical_types
                for i in issues
            )
    ...
    def _generate_recommendations(self, check_name: str,
    ...
    def enforce(self, directory: str = '.', fail_on_warning: bool = False) -> int:
        ...
        # Save JSON report
        report_path = Path(directory) / '.scilint_report.json'
